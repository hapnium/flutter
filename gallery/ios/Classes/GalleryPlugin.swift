...
public class GalleryPlugin: NSObject, FlutterPlugin {
  ...
  private func getMedium(mediumId: String) throws -> [String: Any?] {
    ...
    if (assets.count <= 0) {
      throw NSError(domain: "gallery", code: 404)
    } else {
      ...
      manager.requestImage(
        ...
        resultHandler: { (uiImage: UIImage?, info) in
          guard let image = uiImage else {
            completion(nil, NSError(domain: "gallery", code: 404, userInfo: nil))
            return
          }
            ...
            DispatchQueue.main.async(execute: {
              ...
              if mimeType != nil {
                ...
                if type != mimeType {
                  ...
                  completion(path, NSError(domain: "gallery", code: 404, userInfo: nil))
                  return
                }
              }
          ...
          resultHandler: { (avAsset, avAudioMix, info) in
            DispatchQueue.main.async(execute: {
              do {
                ...
                completion(nil, NSError(domain: "gallery", code: 500, userInfo: nil))
              }
            })
          }
        )
...
//   }
  ...
  private func cachePath() -> URL {
    ...
    let cacheFolder = paths[0].appendingPathComponent("gallery")
  ...
  private func deleteMedium(mediumId: String, completion: @escaping (Bool, Error?) -> Void) {
    ...
    if assets.count <= 0 {
      completion(false, NSError(domain: "gallery", code: 404, userInfo: nil))
    } else {
      let asset: PHAsset = assets[0]
